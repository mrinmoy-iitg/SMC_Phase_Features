...
def modified_group_delay_feature(frames, gamma, rho, num_coeff=12, frame_length=0.025, frame_shift=0.010):
    '''
    # input: 
    #     file_name: path for the waveform. The waveform should have a header
    #     gamma: a parameter to control the shape of modified group delay spectra
    #     rho: a parameter to control the shape of the modified group delay spectra
    ...
    # Please tune gamma and rho for better performance
...
#    print('mgd x-y-spec: ', np.shape(x_spec), np.shape(y_spec))
    ...
    # grp_phase1 = np.divide((np.multiply(np.real(x_spec),np.real(y_spec)) + np.multiply(np.imag(y_spec),np.imag(x_spec))), np.power(np.exp(smooth_spec), (2*gamma)) + 1e-10 )
    grp_phase1_deno = np.power(np.exp(smooth_spec).astype(np.float32), (2*gamma)).astype(np.float32)
    ...
    # grp_phase = np.multiply(np.divide(grp_phase1, np.abs(grp_phase1)+1e-10) , np.power(np.abs(grp_phase1), rho))
    grp_phase = np.multiply(np.divide(grp_phase1, np.abs(grp_phase1)+1e-10) , np.power(np.abs(grp_phase1), rho).astype(np.float32))
...
def compute_mgdcc(PARAMS, frames, fs, gamma=0.3, rho=0.1):
    frame_length = PARAMS['Tw']/1000 # in secs
    frame_shift = PARAMS['Ts']/1000 # in secs
    grp_phase, MGDCC = modified_group_delay_feature(frames, gamma, rho, PARAMS['n_cep'], frame_length, frame_shift)
